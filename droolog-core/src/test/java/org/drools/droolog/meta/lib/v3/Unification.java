...
public class Unification<T> {
    ...
    private <T> Term<T> unifyVariable(Term.Variable<T> left, Term<T> right) {
        if (right instanceof Term.Structure) {
            ...
            Term.Structure<T> n = (Term.Structure<T>) makeStructure();
            Term.Structure<T> t = unify(n, s);
            bindings.put(left, t);
            return t;
    ...
    private Term.Structure<T> makeStructure(Term<?>... terms) {
        return (Term.Structure<T>) PersonMeta.Instance.of(terms);
    }
